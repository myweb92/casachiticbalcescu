...
// The array starts with `const ROOMS: Room[] = [` and ends with `];` before `const REVIEWS: Review[] = [`
const startStr = 'const ROOMS: Room[] = [';
const endStr = 'const REVIEWS: Review[] = [';
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  // find the last `];` before `const REVIEWS:`
  const betweenRoomsAndReviews = appContent.substring(startIndex, endIndex);
  const roomsEnd = betweenRoomsAndReviews.lastIndexOf('];') + 2;

  const after = appContent.substring(startIndex + roomsEnd);
  ...
  console.log('Could not find ROOMS or REVIEWS array in src/App.tsx');
}
