...
// 1. HistorySection.tsx
let hsContent = fs.readFileSync('src/components/HistorySection.tsx', 'utf8');
hsContent = hsContent.replace(
  /<span className="text-\[10px\] font-sans font-bold tracking-\[0\.3em\] uppercase text-\[#A67C52\] block">\s*Preserving Transylvanian Heritage\s*<\/span>/,
  ''
);
fs.writeFileSync('src/components/HistorySection.tsx', hsContent);

// 2. Footer.tsx
let footerContent = fs.readFileSync('src/components/Footer.tsx', 'utf8');
footerContent = footerContent.replace(
  /<a href="#anpc" className="hover:text-\[#A67C52\] transition-colors">\s*ANPC\s*<\/a>/,
...
roomCardContent = roomCardContent.replace(
  /<button\s+onClick=\{[^}]+\}\s+className="bg-\[#A67C52\] hover:bg-\[#8E6A46\] text-white font-sans font-bold text-\[10px\] tracking-widest uppercase py-2\.5 transition-all cursor-pointer"\s+id=\{[^}]+\}\s*>\s*Book Stay\s*<\/button>/g,
  `<a
              href="${url}"
              target="_blank"
              rel="noopener noreferrer"
              className="bg-[#A67C52] hover:bg-[#8E6A46] text-white font-sans font-bold text-[10px] tracking-widest uppercase py-2.5 transition-all cursor-pointer inline-flex items-center justify-center"
...
appContent = appContent.replace(
  /<button\s+onClick=\{[^}]+\}\s+className="w-full sm:w-auto bg-\[#A67C52\] hover:bg-\[#8E6A46\] text-white font-sans font-bold text-xs tracking-\[0\.2em\] uppercase px-8 py-4 rounded-none transition-all hover:scale-\[1\.02\] active:scale-\[0\.98\] cursor-pointer"\s+id="hero-book-now-btn"\s*>\s*Book Your Stay\s*<\/button>/g,
  `<a
              href="${url}"
              target="_blank"
              rel="noopener noreferrer"
